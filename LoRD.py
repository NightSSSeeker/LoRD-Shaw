...
def process_PINT_data(MagIC_directory='MagIC/',AF_min=0,AF_max=180):
    ...
    maxKrv=0.2 # consideration for Lloyd et al.(2021)
    maxBeta=0.1 # consideration for Lloyd et al.(2021)
    maxFresid=0.1 # consideration for Lloyd et al.(2021)
            ...
            if (analysis == 'ordinary') or (analysis == 'best_reg'):
                x1,x2,x3,x4,x5,x6,x7,x8,x9,ss1 =\
                    ts.plot_pint_main(sid,ax2,sid_df,aftrm1,'trm2_star','trm1',\
                                      ...
                                      'krv',minR,minFrac,minSlopeT,maxSlopeT,maxBeta,maxFresid,\
                                      maxKrv,lab_field)
            if (analysis == 'best_krv'):
                x1,x2,x3,x4,x5,x6,x7,x8,x9,ss1 =\
                    ts.plot_pint_main(sid,ax2,sid_df,aftrm1,'trm2_star','trm1',\
...
def LoRD_PRR(R_PRR, P_PRR, K_PRR, Reg_PRR, L_PRR):
    """ Calculate LoRD-Shaw results by the "RR" method
    Paramters
    _________
    ...
    for i in range((len(Reg_Sel)-1)):
        for j in range(i+1,(len(Reg_Sel))): 
            if max(Reg_Sel[j][4],Reg_Sel[i][4])<abs(Reg_Sel[j][0]-Reg_Sel[i][0]):
                ...
                if Label_Count==0:
                    plt.scatter((Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0]), (Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0])*Reg_Sel[i][0]+Reg_Sel[i][1], linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, label='LoRD-Shaw (PRR)', zorder=2)
                else:
    ...
    plt.title('LoRD-Shaw (PINT+R-R)' + '\n' + 'PINT = ' + '%.1f'%np.mean(PINTC) + ' ± ' + '%.1f'%np.std(PINTC) + ' ($\mu$T)',font1, pad=10)
    LoRD_PRR_PINT=pd.DataFrame({'AF cuttings' : Label_Pair, 'PINT' : PINTC})
    LoRD_PRR_PINT.to_csv('csv/LoRD_Shaw_PRR.csv', index=None)
    plt.savefig("Figures/LoRD_Shaw_PRR.pdf", dpi=600)



def LoRD_RR(R_RR, P_RR, K_RR, Reg_RR, Reg_RR_P, L_RR):
    """ Calculate LoRD-Shaw results by the "R-R" method (ABORTED)
    Paramters
    _________
    ...
    for i in range((len(Reg_Sel)-1)):
        for j in range(i+1,(len(Reg_Sel))): 
            if max(Reg_Sel[j][4],Reg_Sel[i][4])<abs(Reg_Sel[j][0]-Reg_Sel[i][0]):
                ...
                if Label_Count==0:
                    plt.scatter((Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0]), (Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0])*Reg_Sel[i][0]+Reg_Sel[i][1], linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, label='LoRD-Shaw (RR)', zorder=2)
                else:
                    plt.scatter((Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0]), (Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0])*Reg_Sel[i][0]+Reg_Sel[i][1], linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, zorder=2)
                Label_Pair.append([Label_Sel[i],Label_Sel[j]])
                Label_Count=1
    ...
    if len(PINTC)>0:
        plt.scatter(PINTC_R, PINTC, linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, label='LoRD-Shaw (RR)', zorder=2)
    ...
    [label.set_fontname('Times New Roman') for label in labels]
    plt.legend(loc='upper left',prop=font3)
    plt.title('LoRD-Shaw (R-R)' + '\n' + 'PINT = ' + '%.1f'%np.mean(PINTC) + ' ± ' + '%.1f'%np.std(PINTC) + ' ($\mu$T)',font1, pad=10)
    LoRD_RR_PINT=pd.DataFrame({'AF cuttings' : Label_Pair, 'PINT' : PINTC})
    LoRD_RR_PINT.to_csv('csv/LoRD_Shaw_RR.csv', index=None)
    plt.savefig("Figures/LoRD_Shaw_RR.pdf", dpi=600)



def LoRD_PR(R_PR, P_PR, K_PR, Reg_PR, L_PR):
    """ Calculate LoRD-Shaw results by the "PR" method
    Paramters
    _________
    ...
    for i in range((len(Reg_Sel)-1)):
        for j in range(i+1,(len(Reg_Sel))): 
            if max(Reg_Sel[j][4],Reg_Sel[i][4])<abs(Reg_Sel[j][0]-Reg_Sel[i][0]):                
                PINTC.append((Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0])*Reg_Sel[i][0]+Reg_Sel[i][1])
                if Label_Count==0:
                    plt.scatter((Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0]), (Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0])*Reg_Sel[i][0]+Reg_Sel[i][1], linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, label='LoRD-Shaw (PR)', zorder=2)
                else:
                    plt.scatter((Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0]), (Reg_Sel[j][1]-Reg_Sel[i][1])/(Reg_Sel[i][0]-Reg_Sel[j][0])*Reg_Sel[i][0]+Reg_Sel[i][1], linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, zorder=2)
                Label_Count+=1
                Label_Pair.append([Label_Sel[i],Label_Sel[j]])
    ...
    [label.set_fontname('Times New Roman') for label in labels]
    plt.legend(loc='upper left',prop=font3)
    plt.title('LoRD-Shaw (PINT-R)' + '\n' + 'PINT = ' + '%.1f'%np.mean(PINTC) + ' ± ' + '%.1f'%np.std(PINTC) + ' ($\mu$T)',font1, pad=10)
    LoRD_PR_PINT=pd.DataFrame({'AF cuttings' : Label_Pair, 'PINT' : PINTC})
    LoRD_PR_PINT.to_csv('csv/LoRD_Shaw_PR.csv', index=None)
    plt.savefig("Figures/LoRD_Shaw_PR.pdf", dpi=600)



def LoRD_curve_fits(R_CF, P_CF, K_CF, Reg_CF, L_CF):
    """ Calculate LoRD-Shaw curve fit results
    Paramters
    _________
    ...
    for i in range(len(DelR)):
        ...
        for j in range(len(x)):
            if A*B>0 and A>0:
                if y[j]==np.min(y) and x[j]>np.min(R_CF[0]) and x[j]<np.max(R_CF[0]):
            ...
            if A*B>0 and A<0:
                if y[j]==np.max(y) and x[j]>np.min(R_CF[0]) and x[j]<np.max(R_CF[0]):
                    plt.scatter(x[j], y[j], color='black', marker="v", s=50, zorder=2)
                    CF_Cuttings.append(Labels[i+1])
                    PINT_RCF.append(x[j])
                    PINT_CF.append(x[j]*Reg_CF[0][0]+Reg_CF[0][1])
    ...
    if len(PINT_CF)>0:
        plt.scatter(PINT_RCF, PINT_CF, linewidths=0.75, edgecolors='black', c='gold', marker='*', s=100, label='LoRD-Shaw (Curve fits)', zorder=2)
    plt.plot(x, y, linestyle='solid', color='black', markersize=15, label='Baseline regression', zorder=1)
    plt.xlabel('R$_0$/R$_1$ (baseline)',font2)
    plt.ylabel('Paleointensity ($\mu$T)',font2)
    tick_div=5
    ...
    [label.set_fontname('Times New Roman') for label in labels]
    plt.legend(loc='upper left',prop=font3)
    plt.title('LoRD-Shaw (Curve fits)' + '\n' + 'PINT = ' + '%.1f'%np.mean(PINT_CF) + ' ± ' + '%.1f'%np.std(PINT_CF) + ' ($\mu$T)',font1, pad=10)
    LoRD_CF_PINT=pd.DataFrame({'AF cuttings' : CF_Cuttings, 'PINT' : PINT_CF})
    LoRD_CF_PINT.to_csv('csv/LoRD_Shaw_CF.csv', index=None)
    plt.savefig("Figures/LoRD_Shaw_CF.pdf", dpi=600)



def LoRD_Shaw(input_file_1='R_Cuttings', input_file_2='PINT_Sel', method='PRR', curve_fits=False):
    """ Calculate LoRD-Shaw results
    Paramters
    _________
    ...
    if R_len>10:
        ...
        if method=='PRR':
            LoRD_PRR(SelR, SelP, Krv_R, Reg_P, Labels)
        if method=='RR':
            LoRD_RR(SelR, SelP, Krv_R, Reg_R, Reg_P, Labels)
        if method=='PR':
            LoRD_PR(SelR, SelP, Krv_P, Reg_P, Labels)
        if curve_fits:
            LoRD_curve_fits(SelR, SelP, Krv_R, Reg_P, Labels)
